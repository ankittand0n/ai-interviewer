...
export async function GET() {
  try {
    ...
    const archivedCandidates = candidatesData.candidates
      .filter((candidate: Candidate) => candidate.status === 'archived')
    ...
    return NextResponse.json({ candidates: archivedCandidates })
  } catch (error) {
    console.error('Error loading archived candidates:', error)
    return NextResponse.json(
      { error: 'Failed to load archived candidates', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
} 